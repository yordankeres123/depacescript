...
print mess
print "Script by GREEN DRAGON"
print "<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<"
title = raw_input("Judul Untuk Title: ")
print (">>>>>>>>>>>>>>>>>>>>>>>>>>>>>")
heading = raw_input("Hacked by :")
print "<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<"
imagelink = raw_input("masukan link gambar (untuk tengah): ")
print ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>"
bgimage = raw_input("masukan link gambar (untuk background): ")
print "<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<"
message = raw_input("Pesan romantis untuk admina,,>> gunakan kode <br> untuk text selanjutnya yah : ")
print ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>"
textcolor = raw_input("masukan Warna text (bahas linggis :v ) : ")
print "<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<"
youtubeid = raw_input("Musik Code Youtube : ")
print ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>"

#Open the index
fo = open("Gr3enDr4gon.html","w")
